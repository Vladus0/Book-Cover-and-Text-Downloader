...
def get_fantastic_book(args): 
    books_urls = []
    for page in range(args.first, args.last):
        url = f'https://knigofil.org/romance-fiction/page/{page}'
...
def get_fantastic_book_info(book_url): 
...
def main():
    ...
    books_urls = get_fantastic_book(args)
    all_book_info = []
    
    for book_url in books_urls: 
        book_info, book_name = get_fantastic_book_info(book_url)
        response = requests.get(book_url)
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'lxml')
        ...
        if not args.skip_imgs:
            folder='fantstic_images/'
            download_images(img_url, folder='fantstic_images/')
            imgs_name = urlsplit(img_url).path.split('/')[-1]
            book_info.update({'img_src': f'{folder}{imgs_name}'})

        if not args.skip_txt:
            folder='fantstic_books/'
            download_text(txt_link, book_name, folder='fantstic_books/')
            book_info.update({'book_path': f'{folder}{book_name}'})
